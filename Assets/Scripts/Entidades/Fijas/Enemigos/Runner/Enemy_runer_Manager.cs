...
public class Enemy_runer_Manager : MonoBehaviour
{
    ...
    public int actual_runer_enemies = 0;
    ...
    void Update()
    {
        if (actual_runer_enemies <= 0)
        {
            for (int i = 0; i < max; i++)
            {
                ...
                temp.GetComponent<Enemy_runer_Spawn>().manager = this;
            }
            max++;
        }
    }
